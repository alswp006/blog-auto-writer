...
const CATEGORY_EXAMPLES: Record<string, { ko: string[]; en: string }> = {
  restaurant: {
    ko: [
      // 20s — 명사형 종결(~임/~음/~함), 짧은 호흡, 직접적 감탄, 줌마체 어미 금지
      `간판 없는 골목집인데 이게 진짜 미침. 문 여는 순간 고기 냄새 확 때리는데 아 여기 맞다 싶음. 메뉴 네 개. 근데 이런 집이 진짜 잘함ㅋㅋ 삼겹살 올리자마자 지글지글 소리부터 다르고 한 입 먹었는데 겉바속촉에 육즙 터짐. 친구랑 눈 마주치고 말 안 해도 알았음. 여기 또 옴.`,
  ...
  cafe: {
    ko: [
      // 20s
      `여기 찐임. 인스타에서 보고 또 감성카페인가 했는데 들어가니까 다름. 원두 볶는 냄새 바로 때리고 아 직접 로스팅하는 집이구나 함. 아아 시켰는데 산미가 딱 내 취향임. 창가 자리 앉아서 멍 때리기 ㄹㅇ 최고ㅋㅋ 다음에 노트북 들고 와야지.`,
      // 30s
  ...
  accommodation: {
    ko: [
      // 20s
      `체크인할 때 웰컴 드링크 주면서 루프탑 바 시간까지 알려줌. 방 들어가자마자 침대 다이빙함. 이불 폭신한 거 실화..? 바로 못 일어남ㅋㅋ 야경도 살짝 보이고 어메니티 이솝이라 씻고 나니까 나한테서 좋은 향남. 이 가격에 이 정도면 완전 이득임.`,
      // 30s
  ...
  attraction: {
    ko: [
      // 20s
      `안국역 1번 출구에서 걸어가면 5분도 안 걸림. 평일이라 사람 없어서 사진 찍기 개좋음. 돌담길 진짜 예뻐서 걸으면서 계속 찍음ㅋㅋ 소나무 사이로 보이는 전각 미쳤고 향원정 연못 앞에서 한참 서 있었음. 여기 봄에 다시 올 거임.`,
      // 30s
...
- 종결어미는 '~임', '~음', '~함', '~인데', '~했는데', '~좋음' 위주. '~해요/~했어요'는 최소화.
- 'ㅋㅋ', 'ㄹㅇ', '찐', '미침', '실화', '개좋음' 같은 표현 자연스럽게 섞기.
- 감탄은 직접적으로: '이거 진짜 미침', '무조건 가야 됨', '나만 알고 싶은 곳'
- 이모지는 안 써도 됨. 쓰더라도 1~2개만. ^^나 ♡ 같은 건 줌마체라 금지.`,
